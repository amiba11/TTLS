...
class Trainer(AbstractTrainer):
    ...
    def __init__(self, config, model):
        ...
        self.epochs_test = config["epochs_test"]
    ...
    #############################################################################################
    # 1. Test-Time Training (TTT) strategy
    def evaluate_ttt(
        self, eval_data, load_best_model=True, model_file=None, show_progress=True
    ):
        r"""Evaluate the model based on the eval data. And train the model with TTT (Test-Time Training) strategy.
        ...
        for epoch_idx in range(1, self.epochs_test):
            # train
            self.model.train()
            ...
            for batch_idx, batched_data in enumerate(iter_data):
                ...
                interaction, scores, positive_u, positive_i = eval_func(batched_data)
                scores[:, 0] = -100.00 # masked
                loss_func = self.model.entropy_loss

                # loss_func = self.model.calculate_perplexity
                ...
                # for perplexity loss
                # with torch.autocast(device_type=self.device.type, enabled=self.enable_amp):
                #     loss = loss_func(batched_data)

                total_loss = (
                    loss.item() if total_loss is None else total_loss + loss.item()
                )
                self._check_nan(loss)
                scaler = amp.GradScaler(enabled=self.enable_scaler)
                scaler.scale(loss + sync_loss).backward()
            ...
            self._add_train_loss_to_tensorboard(epoch_idx, total_loss)
            self.wandblogger.log_metrics(
                {"test_epoch": epoch_idx, "test_time_train_loss": total_loss},
                head="test_time_train",
            )
            self.logger.info("test_epoch: {}, test_time_train_loss: {}".format(epoch_idx, total_loss))
            if self.gpu_available and show_progress:
                iter_data.set_postfix_str(
                    set_color("GPU RAM: " + get_gpu_usage(self.device), "yellow")
                )

            self._save_checkpoint(epoch_idx, verbose=True) # save ttt model and we find 1 epoch is the best
