...
class Dataset(torch.utils.data.Dataset):
    ...
    ##############################################################################################
    def _split_index_by_leave_four_out(self, grouped_index):
        """Split indexes by strategy leave four out.

        Args:
            grouped_index (list of list of int): Index to be split.
            这是特化的, 留一个valid item, 留三个 new items, 留一个test target.  共5个

        Returns:
            list: List of index that has been split.
        """
        next_index = [[] for _ in range(4)] # train, train_2, valid, test
        for index in grouped_index:
            index = list(index)
            tot_cnt = len(index)
            legal_leave_one_num = min(5, tot_cnt - 1)
            pr = tot_cnt - legal_leave_one_num
            next_index[0].extend(index[:pr]) # train
            # train_2
            for i in range(legal_leave_one_num - 2):
                next_index[2].append(index[pr])
                pr += 1
            next_index[1].append(index[pr]) # valid
            pr += 1
            next_index[3].append(index[pr]) # test
        return next_index
    ##############################################################################################
