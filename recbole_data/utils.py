...
def create_dataset(config):
    ...
    if hasattr(dataset_module, config["model"] + "Dataset"):  # 不运行
...
###########################################################################################################
def data_preparation_for_four(config, dataset):
    dataloaders = load_split_dataloaders(config)
    if dataloaders is not None:
        train_data, train_data_2, valid_data, test_data = dataloaders
        dataset._change_feat_format()
    else:
        model_type = config["MODEL_TYPE"]
        built_datasets = dataset.build()

        train_dataset, train_dataset_2, valid_dataset, test_dataset = built_datasets
        train_sampler, train_sampler_2, valid_sampler, test_sampler = create_samplers_for_four(
            config, dataset, built_datasets
        )
        ...
        if model_type != ModelType.KNOWLEDGE:
            ...
            train_data_2 = get_dataloader(config, "train")(
                config, train_dataset_2, train_sampler_2, shuffle=config["shuffle"]
            )
        else:
            kg_sampler = KGSampler(
                dataset,
            ...
            train_data_2 = get_dataloader(config, "train")(
                config, train_dataset_2, train_sampler_2, kg_sampler, shuffle=True
            )
        ...
        if config["save_dataloaders"]:
            save_split_dataloaders(
                config, dataloaders=(train_data, train_data_2, valid_data, test_data)
            )
    ...
    return train_data, train_data_2, valid_data, test_data


def create_samplers_for_four(config, dataset, built_datasets):
    ...
    train_sampler_2 = base_sampler.set_phase("train") if base_sampler else None
    ...
    return train_sampler, train_sampler_2, valid_sampler, test_sampler
###########################################################################################################
